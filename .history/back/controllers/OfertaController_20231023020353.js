...
//OBTENER PORTADA OFERTA
const obtenerPortadaOferta = (req, res) => {
    try {
        const img = req.params.img;
        const ofertaImgPath = `./uploads/ofertas/${img}`;
        const defaultImgPath = './uploads/default.jpg';

        fs.stat(ofertaImgPath, (err) => {
            const path_img = err ? defaultImgPath : ofertaImgPath;
            res.status(200).sendFile(path.resolve(path_img));
        });
    } catch (error) {
        console.error("Error al obtener la portada de la oferta:", error);
        res.status(500).send("Error interno del servidor");
    }
};

module.exports = obtenerPortadaOferta;
