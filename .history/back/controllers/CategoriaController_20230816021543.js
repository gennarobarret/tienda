...
const actualizar_categoria_admin = async function (req, res) {
    if (req.user) {
        ...
        try {
            let categoria_arr = await Categoria.find({
                _id: { $ne: id },
            ...
            if (categoria_arr.length === 0) {
                if (req.files) {
                    ...
                    res.status(200).send({ data: reg });  // Update category with cover ima
                } else {
                    // Update category without cover image
                    // Update category fields without changing the cover image
                }
            } else {
                res.status(404).send({
