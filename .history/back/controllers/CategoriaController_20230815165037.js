...
function isDuplicateImage(existingImagePath, uploadedFile) {
    ...
    console.log(path.extname(existingImagePath) === path.extname(uploadedImageName), path.extname(existingImagePath));
    console.log(validImageTypes.includes(uploadedImageType), uploadedImageType);
    return (
        existingImageStats.size === uploadedImageSize &&
        path.basename(existingImagePath) === path.extname(uploadedImageName) &&
