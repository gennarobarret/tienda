...
function isDuplicateImage(existingImagePath, uploadedFile) {
    ...
    const uploadedImageStats = fs.statSync(uploadedFile.path);

    // Compare name, size, and type of the images
    if (
        path.basename(existingImagePath) === uploadedFile.originalFilename &&
        existingImageStats.size === uploadedImageStats.size &&
        path.extname(existingImagePath) === path.extname(uploadedFile.originalFilename)
    ) {
        return true; // Images are duplicates
    }
    return false; // Images are not duplicates
}
...
const actualizar_categoria_admin = async function (req, res) {
    ...
    if (req.user) {
        ...
        if (categoria_arr.length === 0) {
            if (req.files) {
                // Check if the uploaded image is a duplicate
                const existingImagePath = path.resolve("./uploads/categorias", portada_name);
