...
const eliminar_categoria_admin = async function (req, res) {
    if (req.user) {
        if (req.user.role == "admin") {
            let id = req.params["id"];
            let categoria = await Categoria.findByIdAndRemove({ _id: id });

            // Assuming you have the filename stored in the categoria object
            let filename = categoria.portada;

            fs.stat('./upload/categorias/' + filename, (err) => {
                if (!err) {
                    fs.unlink('./upload/categorias/' + filename, (err) => {
                        if (err) throw err;
                    });
                }
            });
