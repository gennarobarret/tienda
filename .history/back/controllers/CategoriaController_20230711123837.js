...
const obtener_portada = async function (req, res) {
    ...
    fs.stat("./uploads/ofertasCategorias/" + img, function (err) {
        if (!err) {
            let path_img = "./uploads/ofertasCategorias/" + img;
    ...
    if (req.user) {
        if (req.user.role == "admin") {
            ...
            if (req.files) {
                ...
                fs.stat("./uploads/ofertasCategorias/" + reg.portada, function (err) {
                    if (!err) {
                        fs.unlink("./uploads/ofertasCategorias/" + reg.portada, (err) => {
                            if (err) throw err;
                        });
                    }
                });
