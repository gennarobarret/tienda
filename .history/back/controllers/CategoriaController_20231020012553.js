...
const obtener_categoria_admin = async function (req, res) {
    if (req.user) {
        if (req.user.role == "admin") {
            ...
            res.status(403).send({ message: "NoAccess" }); // 403 Forbidden for unauthorized access
        }
    } else {
        res.status(401).send({ message: "Unauthorized" }); // 401 Unauthorized for unauthenticated users
    }
};
