...
const verificarExistenciaTitulo = async function (titulo) {
    try {
        const categoria = await Categoria.findOne({ titulo: titulo.toLowerCase() });
        return categoria !== null;
    } catch (error) {
        console.error('Error al verificar existencia del título:', error);
        return false;
    }
};
...
const actualizar_categoria_admin = async function (req, res) {
    ...
    if (req.user) {
        ...
        console.log(categoria_arr.length === 0);
