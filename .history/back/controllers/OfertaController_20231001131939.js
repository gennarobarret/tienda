...
const listar_ofertas_admin = async function (req, res) {
    if (req.user) {
        if (req.user.role == "admin") {
            ...
            let reg = await Oferta.find({ nombre: new RegExp(filtro, "i") });
            res.status(200).send({ data: reg });
        } else {
            res.status(500).send({ message: "NoAccess" });
        }
...
const registro_oferta_admin = async function (req, res) {
    try {
        ...
        oferta_arr = await Oferta.find({ nombre: { $regex: new RegExp('^' + nombre + '$', 'i') } });
        ...
        if (oferta_arr.length === 0) {
            try {
                if (req.files && req.files.portada) {
                    ...
                    const existingCategory = await Oferta.findById(id);
