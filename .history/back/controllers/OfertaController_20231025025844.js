...
const obtener_portada_oferta = async function (req, res) {
    try {
        ...
        // Manejar la excepción aquí, por ejemplo, enviando una respuesta de error.
        res.status(500).send("Error al obtener la portada de la oferta: " + error.message);
    }
};
...
const registro_oferta_admin = async function (req, res) {
    try {
        ...
        data.estado_oferta = data.estado_oferta === 'undefined' ? true : data.estado_oferta === 'true';
        // Ajustar las fechas aquí antes de crear el objeto 'data'
        if (data.inicio_oferta) {
            data.inicio_oferta = new Date(data.inicio_oferta).toLocaleString();
        }
        if (data.fin_oferta) {
            data.fin_oferta = new Date(data.fin_oferta).toLocaleString();
        }
