...
const registro_categoria_admin = async function (req, res) {
    try {
        let data = req.body;
        // Check if data.nombre_categoria is defined and a string
        if (data.nombre_categoria && typeof data.nombre_categoria === 'string') {
            ...
            // Handle the case where data.nombre_categoria is undefined or not a string
            res.status(400).send({ message: "Invalid input for nombre_categoria" });
        }
    } catch (error) {
        console.error(error);
        ...
        if (req.user.role == "admin") {
            ...
            try {
                let reg = await Categoria
                    ...
                    // .populate('subcategorias') // Esto asume que tienes una propiedad 'subcategorias' en tu modelo de Categoria para relacionarlas
    ...
    try {
        if (req.user) {
            if (req.user.role === "admin") {
                ...
                if (categoria) {
                    // Busca y elimina las subcategorías relacionadas
