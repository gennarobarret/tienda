...
const Categoria = require("../models/oferta");
const fs = require("fs");
...
const registro_oferta_admin = async function (req, res) {
    try {
        ...
        oferta_arr = await Categoria.find({ titulo: { $regex: new RegExp('^' + titulo + '$', 'i') } });
...
const actualizar_oferta_admin = async function (req, res) {
    ...
    if (req.user) {
        ...
        console.log(data);
        const titulo = data.titulo.toLowerCase();
        oferta_arr = await Categoria.find({
            _id: { $ne: id },
            ...
            try {
                if (req.files && req.files.portada) {
                    ...
                    if (existingCategory && existingCategory.portada) {
                        const existingPortadaPath = `uploads/ofertas/${existingCategory.portada}`;
...
const eliminar_oferta_admin = async function (req, res) {
    if (req.user) {
        if (req.user.role === "admin") {
            ...
            const oferta = await Categoria.findByIdAndRemove({ _id: id });
            // console.log(oferta);
            if (oferta) {
                const imageName = oferta.portada;
                if (imageName) {
