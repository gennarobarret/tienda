...
function isDuplicateImage(existingImagePath, uploadedFile) {
    ...
    console.log(existingImageStats.size === uploadedImageSize);
    console.log(path.extname(existingImagePath) === path.extname(uploadedImageName));
    console.log(validImageTypes.includes(uploadedImageType));
    return (
        ...
        if (categoria_arr.length === 0) {
            if (req.files) {
                ...
                if (fs.existsSync(existingImagePath) && isDuplicateImage(existingImagePath, req.files.portada)) {
                    return res.status(407).send({ message: "Duplicate image detected." });
                }
