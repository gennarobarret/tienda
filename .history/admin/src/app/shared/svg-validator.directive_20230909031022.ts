...
function validateSVG(
  ...
  if (svgContent) {
    ...
    const svgPathRegex = /<path\s[^>]*d="([^"]*)"[^>]*>/i;
    const match = svgContent.match(svgPathRegex);

    if (match) {
      const dAttribute = match[1]; // El valor del atributo d
      // Validar que las comillas dobles estén presentes al comienzo y al final
      if (/^\s*".*"\s*$/.test(dAttribute)) {
        // Eliminar los espacios en blanco y las comillas dobles al comienzo y al final
        const dAttributeValue = dAttribute.trim().slice(1, -1);
        // Validar el atributo d utilizando el patrón regex adecuado
        if (!validAttributes['d'].test(dAttributeValue)) {
          showErrorMessage("El atributo d no es válido.");
          return false;
        }
      } else {
        showErrorMessage("Las comillas dobles faltan al comienzo o al final del atributo d.");
        return false;
      }
    } else {
      showErrorMessage("La etiqueta <path> no se encuentra en el contenido SVG.");
      return false;
    }
