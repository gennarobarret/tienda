...
export function svgValidator(sanitizer: DomSanitizer): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const svgContent: string = control.value;
    // Parse the SVG content and validate its elements and attributes here
    const isValid = validateSVG(svgContent, sanitizer); // Pass sanitizer as an argument
...
function validateSVG(svgContent: string, sanitizer: DomSanitizer): boolean {
  // Your SVG validation logic here

  // After successful validation, update trustedSvg
  trustedSvg = this.sanitizeSvg(svgContent, sanitizer); // Use 'this.sanitizeSvg' here
...
export class SvgValidatorDirective implements Validator {
  ...
  public validate(control: AbstractControl): ValidationErrors | null {
    const svgContent: string = control.value;
    // Parse the SVG content and validate its elements and attributes here
    const isValid = validateSVG(svgContent, this.sanitizer); // Pass sanitizer as an argument
  ...
  private sanitizeSvg(svgContent: string, sanitizer: DomSanitizer): SafeHtml | null {
    try {
...
//     trustedSvg = sanitizeSvg(svgContent);
