...
import { Observable, throwError } from 'rxjs'; // Agregamos throwError
import { switchMap } from 'rxjs/operators'; // Agregamos switchMap
...
export class CategoriaService {
  ...
  verificarExistenciaTitulo(titulo: string, token: any): Observable<boolean> {
    let headers = new HttpHeaders({ 'Authorization': token });
    return this._http.get<boolean>(this.url + 'verificar_existencia_titulo/' + titulo, { headers: headers });
  }

  registro_categoria_admin(data: any, file: any, token: any): Observable<any> {
    return this.verificarExistenciaTitulo(data.titulo, token).pipe(
      switchMap((tituloExiste: boolean) => { // Corregimos la declaración de tipo
        if (tituloExiste) {
          return throwError('El título de categoría ya existe');
        } else {
          let headers = new HttpHeaders({ 'Authorization': token });
