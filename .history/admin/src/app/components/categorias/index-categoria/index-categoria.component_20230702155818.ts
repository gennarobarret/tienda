...
export class IndexCategoriaComponent implements OnInit {
  ...
  init_Data() {
    this._categoriaService
      .listar_categorias_filtro_admin(null, null, this.token)
      .subscribe(
        (response) => {
  ...
  filtro(tipo: any) {
    ...
    if (tipo == 'company') {
      if (this.filtro_company) {
        this._categoriaService
          .listar_categorias_filtro_admin(tipo, this.filtro_company, this.token)
          .subscribe(
            (response) => {
              this.categorias = response.data;
              this.load_data = false;
      ...
      if (this.filtro_correo) {
        this._categoriaService
          .listar_categorias_filtro_admin(tipo, this.filtro_correo, this.token)
          .subscribe(
            (response) => {
              this.categorias = response.data;
              this.load_data = false;
