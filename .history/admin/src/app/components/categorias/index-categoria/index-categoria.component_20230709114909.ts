...
import { ProductoService } from 'src/app/services/categoria.service';
...
export class IndexProductoComponent implements OnInit {
  ...
  constructor(
    private _categoriaService: ProductoService
  ) {
  ...
  init_data() {
    this._categoriaService.listar_productos_admin(this.filtro, this.token).subscribe(
      response => {
  ...
  eliminar(id: any) {
    this.load_btn = true;
    this._categoriaService.eliminar_producto_admin(id, this.token).subscribe(
      response => {
        iziToast.show({
          title: 'SUCCESS',
          titleColor: '#1DC74C',
