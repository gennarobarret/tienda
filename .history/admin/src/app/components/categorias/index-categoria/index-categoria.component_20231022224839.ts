...
export class IndexCategoriaComponent implements OnInit {
  ...
  sortOffer() {
    ...
    this.categorias.sort((a, b) => {
      const updatedAtA = a.ofertas_subcategoria[0]?.updatedAt || '';
      const updatedAtB = b.ofertas_subcategoria[0]?.updatedAt || '';

      const result = updatedAtA.localeCompare(updatedAtB);
