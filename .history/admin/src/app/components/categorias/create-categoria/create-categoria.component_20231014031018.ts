...
export class CreateCategoriaComponent implements OnInit {
    ...
    exchangeOffers() {
        ...
        if (selectedOffers && selectedOffers.length > 0) {
            ...
            // Create a deep copy of ofertasDisponibles array
            const ofertasDisponiblesValue = [...ofertasDisponibles.value];

            // Move selected offers from ofertasDisponibles to ofertasAplicadas
            selectedOffers.forEach((offer: any) => {
                const index = ofertasDisponiblesValue.findIndex((o: any) => o === offer);
                if (index !== -1) {
                    console.log('Moving offer from ofertasDisponibles to ofertasAplicadas:', offer);
                    ofertasDisponiblesValue.splice(index, 1);
                    ofertasAplicadas.value.push(offer);
                }
            });

            // Update form controls with the modified values
            ofertasDisponibles.setValue(ofertasDisponiblesValue);
            ofertasAplicadas.updateValueAndValidity();

            console.log('ofertasDisponibles:', ofertasDisponiblesValue);
