        ...
        if (ofertaControl && descuentoOfertaControl && fechaOfertaControl && portadaControl) {
            ofertaControl.valueChanges.subscribe((ofertaValue) => {
                if (ofertaValue === false) {
                    // Marcar campos como "readonly" en lugar de deshabilitarlos
                    this.categoriaForm.get('descuento_oferta').readonly();
                    this.categoriaForm.get('fin_oferta').readonly();
                    this.categoriaForm.get('portada').readonly();
                }
            });
        }
