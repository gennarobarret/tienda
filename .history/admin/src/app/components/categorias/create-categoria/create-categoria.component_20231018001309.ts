...
export class CreateCategoriaComponent implements OnInit {
    ...
    // selectedOffers: any[] = [];
    // dropdownSettings = {
    //     singleSelection: false, // Set to true for single selection, false for multiple selection
    //     idField: '_id',
    //     textField: 'nombre_oferta',
    //     selectAllText: 'Select All',
    //     unSelectAllText: 'Unselect All',
    //     itemsShowLimit: 3, // Number of items to display in the dropdown before truncating
        ...
        this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
            (response: any) => {
                ...
                this.categoriaForm.patchValue({
                    ofertasDisponibles: this.ofertasDisponibles.map((offer: any) => offer._id)
                });
    ...
    //     // Remove duplicates
    //     this.ofertasDisponibles = this.ofertasDisponibles.filter((offer, index, self) =>
    //         index === self.findIndex((o) => o._id === offer._id)
    //     );
    //     this.ofertasAplicadas = this.ofertasAplicadas.filter((offer, index, self) =>
    //         index === self.findIndex((o) => o._id === offer._id)
    //     );

    //     // Update the form controls with the updated values
    //     this.categoriaForm.patchValue({
