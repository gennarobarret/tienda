...
export class CreateCategoriaComponent implements OnInit {
    ...
    agregarOfertaAplicada(): void {
        ...
        // Create a copy of ofertas_aplicadas
        const nuevasOfertasAplicadas = [...this.ofertas_aplicadas];

        // Move the selected offers from disponibles to aplicadas
        for (const ofertaId of ofertasSeleccionadas) {
            const oferta = this.ofertas_disponibles.find(o => o.id === ofertaId);
            if (oferta) {
                nuevasOfertasAplicadas.push(oferta);
            }
        }

        this.ofertas_aplicadas = nuevasOfertasAplicadas;
