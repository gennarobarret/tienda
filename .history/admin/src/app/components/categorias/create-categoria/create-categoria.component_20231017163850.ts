...
export class CreateCategoriaComponent implements OnInit {
    ...
    moveSelectedOffers(action: 'toApplied' | 'toAvailable'): void {
        ...
        // Mantener un seguimiento de las selecciones en variables independientes
        const previouslySelectedAvailableIds = [...selectedOfferAvailableIds];
        const previouslySelectedApplyIds = [...selectedOfferApplyIds];

        if (action === 'toApplied') {
            // Mover las ofertas seleccionadas de ofertasDisponibles a ofertasAplicadas
            const selectedOffers = this.ofertasDisponibles.filter(offer => previouslySelectedAvailableIds.includes(offer._id));
            this.ofertasAplicadas = this.ofertasAplicadas.concat(selectedOffers);
            this.ofertasDisponibles = this.ofertasDisponibles.filter(offer => !previouslySelectedAvailableIds.includes(offer._id));
        } else if (action === 'toAvailable') {
            // Mover las ofertas seleccionadas de ofertasAplicadas de vuelta a ofertasDisponibles
            const selectedOffers = this.ofertasAplicadas.filter(offer => previouslySelectedApplyIds.includes(offer._id));
            this.ofertasDisponibles = this.ofertasDisponibles.concat(selectedOffers);
            this.ofertasAplicadas = this.ofertasAplicadas.filter(offer => !previouslySelectedApplyIds.includes(offer._id));
        }

        // Actualizar los controles del formulario con las variables independientes
