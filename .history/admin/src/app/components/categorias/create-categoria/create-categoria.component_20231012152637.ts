...
export class CreateCategoriaComponent implements OnInit {
    ...
    agregarOfertaAplicada(): void {
        ...
        // Filter and move the selected offers from disponibles to aplicadas
        const ofertasMovidas = this.ofertas_disponibles.filter(oferta => ofertasSeleccionadas.includes(oferta.id));
        this.ofertas_aplicadas.push(...ofertasMovidas);

        // Remove the selected offers from disponibles
        this.ofertas_disponibles = this.ofertas_disponibles.filter(oferta => !ofertasSeleccionadas.includes(oferta.id));
