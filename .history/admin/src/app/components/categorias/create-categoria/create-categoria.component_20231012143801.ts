...
export class CreateCategoriaComponent implements OnInit {
    ...
    initializeForm(): void {
        ...
        this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
            response => {
                ...
                // });
                console.log(this.ofertas_disponibles);
    ...
    agregarOfertaAplicada(): void {
        if (this.ofertas_disponibles) {
            // Find the selected offer from the list of available offers
            const selectedOffer = this.ofertas_categoria.find(offer => offer.id === this.ofertas_disponibles);
            if (selectedOffer) {
                ...
                this.ofertas_aplicadas.push(selectedOffer);
                console.log(this.ofertas_aplicadas);
                // Clear the selection
                this.ofertas_disponibles = null;
            }
        }
    }

    quitarOfertaAplicada(): void {
        if (this.ofertas_aplicadas.length > 0) {
            // Find the selected offer from "Ofertas Aplicadas"
            const selectedOffer = this.ofertas_aplicadas[0];
            if (selectedOffer) {
                // Remove the selected offer from the list of "Ofertas Aplicadas"
                this.ofertas_aplicadas = this.ofertas_aplicadas.filter(offer => offer.id !== selectedOffer.id);
