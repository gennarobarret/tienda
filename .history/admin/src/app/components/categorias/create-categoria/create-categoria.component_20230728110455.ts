...
export class CreateCategoriaComponent {
  ...
  validateSvgCode() {
    ...
    // Verificar si svgCode es nulo, no es una cadena o es una cadena vacía antes de continuar
    if (svgCode === null || typeof svgCode !== 'string' || svgCode.trim() === '') {
      this.showViewer = false;
      this.showInvalidSvg = false; // No mostrar el mensaje de error si svgCode está vacío
      this.sanitizedSvgCode = '';
      this.categoriaForm.controls['icono'].markAsTouched();
      return; // Salir de la función ya que no hay código SVG válido para validar
    }
