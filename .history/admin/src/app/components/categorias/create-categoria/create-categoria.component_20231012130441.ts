...
export class CreateCategoriaComponent implements OnInit {
    ...
    agregarOfertaAplicada(): void {
        const selectedOfferId = this.categoriaForm.get('ofertas_categoria').value;

        if (selectedOfferId) {
            // Find the selected offer from the list of offers
            const selectedOffer = this.ofertas_categoria.find(offer => offer.id === selectedOfferId);

            if (selectedOffer) {
                // Add the selected offer to the "Ofertas Aplicadas" list
                // You can use an array to store the selected offers
                // For example, create a property in your component to store them:
                // this.selectedOfertasAplicadas.push(selectedOffer);

                // Clear the selected option in the <select>
                this.categoriaForm.get('ofertas_categoria').reset();
            }
        }
    }
