...
export class CreateCategoriaComponent {
  ...
  limitDescuentoOferta() {
    let descuentoOferta = parseInt(this.categoria['descuento_oferta']);

    if (isNaN(descuentoOferta) || descuentoOferta < 0 || isNaN(parseInt(this.categoria['descuento_oferta']))) {
      descuentoOferta = 0;
    } else if (descuentoOferta > 100) {
      descuentoOferta = 100;
    }

    this.categoria['descuento_oferta'] = descuentoOferta.toString().slice(0, 3);
  }
  ...
  registro() {
    ...
    if (this.categoriaForm.valid) {
      if (this.categoria.oferta === 0 && !this.file) {
        iziToast.show({
          title: 'ERROR',
          titleColor: '#FF0000',
          color: '#FFF',
