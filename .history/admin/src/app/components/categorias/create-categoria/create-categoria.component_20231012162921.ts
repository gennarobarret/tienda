...
export class CreateCategoriaComponent implements OnInit {
    ...
    agregarOfertaAplicada(): void {
        ...
        console.log('Selected offers from "Ofertas Disponibles":', ofertasSeleccionadas);
    }
    ...
    captureSelectedOffers(event: Event): void {
        const selectedOptionElements = (event.target as HTMLSelectElement).selectedOptions;
        const selectedOffers = Array.from(selectedOptionElements).map(option => option.value);

        // Update the form control with the selected offers
        this.categoriaForm.get('ofertas_disponibles')!.setValue(selectedOffers);
    }
