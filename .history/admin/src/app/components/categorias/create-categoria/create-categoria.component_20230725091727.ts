...
export class CreateCategoriaComponent {
  ...
  resetForm() {
    ...
    this.categoria = {
      estado: '', // Aquí debes colocar el valor por defecto deseado para el campo 'estado'
      oferta: '', // Aquí debes colocar el valor por defecto deseado para el campo 'oferta'
    };

    // Establecer explícitamente los valores predeterminados en los selectores
    this.categoriaForm.form.get('estado')?.setValue('', { onlySelf: true });
    this.categoriaForm.form.get('oferta')?.setValue('', { onlySelf: true });

    // Reset the image to the default value
    ...
    if (fileInput) {
      fileInput.value = '';
      // Set the label text back to "Seleccionar imagen"
      const label = document.getElementById('input-img_oferta') as HTMLLabelElement;
      if (label) {
