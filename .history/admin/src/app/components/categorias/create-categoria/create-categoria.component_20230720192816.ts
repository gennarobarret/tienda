...
export class CreateCategoriaComponent {
  ...
  ofertaInactive() {
    if (!this.categoria) {
      return; // Exit early if categoria is null or undefined
    }
    // If the "Oferta" status is set to "Inactivo" (0)
    if (this.categoria.oferta === '0') {
      // Set the ofeta value to '0' and hide the error notification
      this.categoria.descuento_oferta = '0';
      // return bolean value descuento_oferta as false
      this.showNotificationError = false;
    } else {
      // If the "Oferta" status is set to "Activo" (1) or any other value
      // Check if the discount value is '0' to show the error notification
      this.categoria.descuento_oferta = this.categoria.descuento_oferta.trim();
