...
export class CreateCategoriaComponent implements OnInit {
    ...
    ofertas: { nombre_oferta: string, _id: string, descuento_oferta: number }[] = [];
    ofertas_disponibles: { nombre_oferta: string, _id: string, descuento_oferta: number }[] = []; // Array to hold available ofertas
    ofertas_aplicadas: { nombre_oferta: string, _id: string, descuento_oferta: number }[] = [];   // Array to hold applied ofertas
    ...
    initializeForm(): void {
        this.categoriaForm = this.fb.group({
            ...
            ofertasDisponibles: this.fb.array([]),
            ofertasSeleccionadas: this.fb.array([])
        });
    ...
    moverSeleccionadasAOfertas() {
        const seleccionadasArray = this.categoriaForm.get('ofertasSeleccionadas') as FormArray;
        const disponiblesArray = this.categoriaForm.get('ofertasDisponibles') as FormArray;

        const seleccionadas = seleccionadasArray.value;
        for (const id of seleccionadas) {
            const index = seleccionadasArray.value.indexOf(id);
            seleccionadasArray.removeAt(index);
            disponiblesArray.push(this.fb.control(id));
        }
    }

    moverDisponiblesASeleccionadas() {
        const seleccionadasArray = this.categoriaForm.get('ofertasSeleccionadas') as FormArray;
        const disponiblesArray = this.categoriaForm.get('ofertasDisponibles') as FormArray;

        const disponibles = disponiblesArray.value;
        for (const id of disponibles) {
            const index = disponiblesArray.value.indexOf(id);
            disponiblesArray.removeAt(index);
            seleccionadasArray.push(this.fb.control(id));
        }
    }
