...
export class CreateCategoriaComponent implements OnInit {
    ...
    initializeForm(): void {
        ...
        this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
            response => {
                ...
                // });
                // this.ofertas_categoria = this.ofertasAplicadas;
    ...
    agregarOfertaAplicada(): void {
        ...
        // Mueve las ofertas seleccionadas de disponibles a aplicadas
        for (const ofertaId of ofertasSeleccionadas) {
            const oferta = this.ofertasDisponibles.find(o => o.id === ofertaId);
            if (oferta) {
                this.ofertasAplicadas.push(oferta);
                this.ofertasDisponibles = this.ofertasDisponibles.filter(o => o.id !== ofertaId);
            }
        }

        // Actualiza los formularios y los arreglos
        this.categoriaForm.patchValue({ ofertas_disponibles: this.ofertasDisponibles });
        this.categoriaForm.patchValue({ ofertas_aplicadas: this.ofertasAplicadas });
    }
