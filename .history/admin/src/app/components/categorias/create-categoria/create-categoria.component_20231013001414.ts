...
export class CreateCategoriaComponent implements OnInit {
    ...
    ofertas: { nombre_oferta: string }[] = [];
    ofertas_disponibles: { nombre_oferta: string }[] = []; // Array to hold available ofertas
    ofertas_aplicadas: { nombre_oferta: string }[] = [];   // Array to hold applied ofertas
    ...
    initializeForm(): void {
        this.categoriaForm = this.fb.group({
            ...
            ofertas_disponibles: this.fb.control([]), // Initialize as an empty array
            ofertas_aplicadas: this.fb.control([]),   // Initialize as an empty array
            ofertas_categorias: this.fb.array([]),     // Initialize as a FormArray
        });
    ...
    moveOfferToApplied(oferta: { id: number, nombre_oferta: string }) {
        // Add the offer ID to ofertas_categorias
        const ofertasCategorias = this.categoriaForm.get('ofertas_categorias') as FormArray;
        ofertasCategorias.push(new FormControl(oferta.id));

        // Move the offer to ofertas_aplicadas
        this.ofertas_aplicadas.push(oferta);

        // Remove the offer from ofertas_disponibles
        const index = this.ofertas_disponibles.findIndex(o => o.id === oferta.id);
        if (index !== -1) {
            this.ofertas_disponibles.splice(index, 1);
        }
    }

    moveOfferToAvailable(oferta: { id: number, nombre_oferta: string }) {
        // Remove the offer ID from ofertas_categorias
        const ofertasCategorias = this.categoriaForm.get('ofertas_categorias') as FormArray;
        const idToRemove = oferta.id;
        const idIndex = ofertasCategorias.controls.findIndex(control => control.value === idToRemove);
        if (idIndex >= 0) {
            ofertasCategorias.removeAt(idIndex);
        }

        // Move the offer back to ofertas_disponibles
        this.ofertas_disponibles.push(oferta);

        // Remove the offer from ofertas_aplicadas
        const index = this.ofertas_aplicadas.findIndex(o => o.id === oferta.id);
        if (index !== -1) {
