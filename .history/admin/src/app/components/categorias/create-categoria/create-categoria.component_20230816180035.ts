  ...
  registro() {
    ...
    if (this.categoriaForm.valid) {
      if (this.file === undefined && this.categoria.oferta === '1') {
        ...
        // this.categoria.titulo = this.filtro;
        this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
          (categorias) => {
            this.categorias = categorias.data;
            // Recorrer y obtener los títulos
            const titulos = this.categorias.map((element: any) => element.titulo);

            // Verificar si existe el título "CATEGORIA A"
            const tituloBuscado = this.categoria.titulo;
            const existeTitulo = titulos.includes(tituloBuscado);

            if (existeTitulo) {
              console.log('El título existe');
              // Aquí puedes mostrar un mensaje o realizar la lógica correspondiente
            } else {
              console.log('El título no existe');
              // Aquí puedes mostrar un mensaje o realizar la lógica correspondiente
            }
