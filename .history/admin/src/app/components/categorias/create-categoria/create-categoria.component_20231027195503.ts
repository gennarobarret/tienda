            ...
            (categoriasResponse: any) => {
                ...
                this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
                    (ofertasResponse: any) => {
                        ...
                        // Filter out offers that are in the past
                        const currentDate = new Date(); // Get the current date
                        this.ofertasDisponibles = this.ofertasDisponibles.filter(oferta => {
                            const endDate = new Date(oferta.fecha_fin);
                            console.log(endDate);// Assuming oferta.fecha_fin is a date field in the offer object
                            return !appliedOfferIds.includes(oferta._id) && endDate > currentDate;
                        });
