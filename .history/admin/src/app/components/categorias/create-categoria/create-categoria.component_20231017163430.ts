...
export class CreateCategoriaComponent implements OnInit {
    ...
    moveSelectedOffers(action: 'toApplied' | 'toAvailable'): void {
        ...
        if (action === 'toApplied') {
            // Mover las ofertas seleccionadas de ofertasDisponibles a ofertasAplicadas
            const selectedOfferIds = selectedOfferAvailableIds.map(String);  // Convert to strings for comparison
            const selectedOffers = this.ofertasDisponibles.filter((offer) => selectedOfferIds.includes(offer._id));
            this.ofertasAplicadas = this.ofertasAplicadas.concat(selectedOffers);
            this.ofertasDisponibles = this.ofertasDisponibles.filter((offer) => !selectedOfferIds.includes(offer._id));
        } else if (action === 'toAvailable') {
            // Mover las ofertas seleccionadas de ofertasAplicadas de vuelta a ofertasDisponibles
            const selectedOfferIds = selectedOfferApplyIds.map(String);  // Convert to strings for comparison
            const selectedOffers = this.ofertasAplicadas.filter((offer) => selectedOfferIds.includes(offer._id));
            this.ofertasDisponibles = this.ofertasDisponibles.concat(selectedOffers);
            this.ofertasAplicadas = this.ofertasAplicadas.filter((offer) => !selectedOfferIds.includes(offer._id));
        }

        // Obtener los valores previamente seleccionados sin cambiar el estado de selección
        const previouslySelectedAvailableIds = selectedOfferAvailableIds.filter((id: string) => this.ofertasDisponibles.some((offer) => offer._id === id));
        console.log(previouslySelectedAvailableIds);
        const previouslySelectedApplyIds = selectedOfferApplyIds.filter((id: string) => this.ofertasAplicadas.some((offer) => offer._id === id));

        // Actualizar los controles del formulario con los valores previamente seleccionados
        this.categoriaForm.get('ofertasAplicadas')!.patchValue(previouslySelectedApplyIds);
        this.categoriaForm.get('ofertasDisponibles')!.patchValue(previouslySelectedAvailableIds);
    }
