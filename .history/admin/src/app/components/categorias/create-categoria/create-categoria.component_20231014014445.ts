...
export class CreateCategoriaComponent implements OnInit {
    ...
    deselectAll() {
        const ofertasAplicadas = this.categoriaForm.get('ofertasAplicadas') as FormArray;
        const ofertasDisponibles = this.categoriaForm.get('ofertasDisponibles') as FormArray;

        // Iterate through selected options in "Ofertas Aplicadas" and move them to "Ofertas Disponibles"
        ofertasAplicadas.value.forEach((ofertaId: string) => {
            const index = ofertasAplicadas.value.indexOf(ofertaId);
            ofertasAplicadas.removeAt(index); // Remove from "Ofertas Aplicadas"
            ofertasDisponibles.push(new FormControl(ofertaId)); // Add to "Ofertas Disponibles"
        });
    }
