...
export class CreateCategoriaComponent implements OnInit {
    ...
    ofertas_disponibles = []; // Array to hold available offers
    ofertas_aplicadas = [];   // Array to hold applied offers
    ...
    // Function to move an offer to the ofertas_aplicadas array
    moveOfferToApplied(offer) {
        this.ofertas_aplicadas.push(offer);
        const index = this.ofertas_disponibles.indexOf(offer);
        if (index !== -1) {
            this.ofertas_disponibles.splice(index, 1);
        }
    }

    // Function to move an offer back to the ofertas_disponibles array
    moveOfferToAvailable(offer) {
        this.ofertas_disponibles.push(offer);
        const index = this.ofertas_aplicadas.indexOf(offer);
        if (index !== -1) {
