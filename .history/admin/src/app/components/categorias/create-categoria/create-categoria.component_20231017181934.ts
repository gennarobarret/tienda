...
export class CreateCategoriaComponent implements OnInit {
    @ViewChild('ofertasDisponiblesSelect') ofertasDisponiblesSelect: ElementRef;
    @ViewChild('ofertasAplicadasSelect') ofertasAplicadasSelect: ElementRef;
    ...
    moveSelectedOffers(action: 'toApplied' | 'toAvailable'): void {
        const ofertasDisponiblesSelect = this.fileInput.nativeElement;
        const ofertasAplicadasSelect = this.fileInput.nativeElement;

        const selectedOfferAvailableIds = Array.from(ofertasDisponiblesSelect.selectedOptions).map(option => option.value);
        const selectedOfferApplyIds = Array.from(ofertasAplicadasSelect.selectedOptions).map(option => option.value);
        ...
        //     ofertasDisponibles: this.ofertasDisponibles.map(offer => offer._id),
        //     ofertasAplicadas: this.ofertasAplicadas.map(offer => offer._id)
        // });
    }

    onSelectChange(event: Event, target: 'ofertasDisponibles' | 'ofertasAplicadas'): void {
        const selectElement = target === 'ofertasDisponibles' ? this.ofertasDisponiblesSelect.nativeElement : this.ofertasAplicadasSelect.nativeElement;

        const selectedOptionValues = Array.from(selectElement.selectedOptions).map(option => option.value);

        if (target === 'ofertasDisponibles') {
            // Update this.ofertasDisponibles with selectedOptionValues
            // ...
        } else if (target === 'ofertasAplicadas') {
            // Update this.ofertasAplicadas with selectedOptionValues
            // ...
        }
