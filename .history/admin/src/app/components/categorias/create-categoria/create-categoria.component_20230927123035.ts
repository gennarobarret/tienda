...
export class CreateCategoriaComponent implements OnInit {
    ...
    subscribeToFormChanges(): void {
        ...
        if (ofertaControl && descuentoOfertaControl && fechaOfertaControl && portadaControl) {
            ...
            // Subscribe to "oferta" Field Changes:
            ofertaControl.valueChanges.subscribe((ofertaValue) => {
                ...
                [descuentoOfertaControl, fechaOfertaControl, portadaControl].forEach(control => {
                    if (control) {
                        const formGroupName = 'categoriaForm'; // Replace with the actual name of your form group
                        const element = document.querySelector(`[formControlName="${control.descuento_oferta}"]`);

                        if (element) {
                            const formControl = this.categoriaForm.get(control.name);
                            if (formControl) {
                                if (isOfertaFalse) {
                                    element.setAttribute('readonly', 'true'); // Set readonly attribute
                                    formControl.disable(); // Disable the control
                                } else {
                                    element.removeAttribute('readonly'); // Remove readonly attribute
                                    formControl.enable(); // Enable the control
                                }
                            }
                        }
                    }
