...
export class CreateCategoriaComponent implements OnInit {
    ...
    initializeForm(): void {
        ...
        this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
            (categoriasResponse: any) => {
                ...
                console.log('ofertasAplicadas', this.ofertasAplicadas);
                ...
                this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
                    (ofertasResponse: any) => {
                        ...
                        this.ofertasAplicadas.forEach(categoria => {
                            console.log('Ofertas de la categoría:', categoria.nombre_categoria);
                            if (Array.isArray(categoria.ofertas_categoria)) {
                                // Filtra las ofertas que están disponibles y que también existen en la lista de ofertas
                                const ofertasDisponibles = categoria.ofertas_categoria
                                    .filter((ofertaCategoria: any) => {
                                        return ofertaCategoria._id && ofertaCategoria.estado_oferta &&
                                            this.ofertas.some(oferta => oferta._id === ofertaCategoria._id);
                                    });
                                console.log('Ofertas disponibles en esta categoría:', ofertasDisponibles);
                                categoria.ofertas_categoria = ofertasDisponibles;
                            }
                        });
    ...
    public registro(): void {
        ...
        console.log(formValue.ofertas_categoria);
