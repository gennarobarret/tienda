...
export class CreateCategoriaComponent implements OnInit {
    ...
    //     console.log(ofertasAplicadas);
    //     this.categoriaForm.get('ofertasDisponibles')?.setValue([...(this.categoriaForm.get('ofertasDisponibles')?.value || []), ...ofertasAplicadas]);
    //     this.categoriaForm.get('ofertasAplicadas')?.setValue([]);
    // }

    exchangeOffers() {
        const selectedOffers = this.categoriaForm.get('ofertasDisponibles')!.value;

        if (selectedOffers && selectedOffers.length > 0) {
            const ofertasDisponibles = this.categoriaForm.get('ofertasDisponibles')!;
            const ofertasAplicadas = this.categoriaForm.get('ofertasAplicadas')!;

            // Move selected offers from ofertasDisponibles to ofertasAplicadas
            selectedOffers.forEach((offer: any) => {
                const index = ofertasDisponibles.value.findIndex((o: any) => o._id === offer._id);
                if (index !== -1) {
                    console.log('Moving offer from ofertasDisponibles to ofertasAplicadas:', offer);
                    ofertasDisponibles.value.splice(index, 1);
                    ofertasAplicadas.value.push(offer);
                }
            });

            // Update form controls with the modified values
            ofertasDisponibles.updateValueAndValidity();
            ofertasAplicadas.updateValueAndValidity();

            ofertasDisponibles.setValue(ofertasDisponibles.value); // Update the value
            ofertasAplicadas.setValue(ofertasAplicadas.value); // Update the value

            console.log('ofertasDisponibles:', ofertasDisponibles.value);
            console.log('ofertasAplicadas:', ofertasAplicadas.value);
        }
    }
    ...
    // exchangeOffers() {
    //     // Obtén las ofertas seleccionadas de ofertasDisponibles
    //     const selectedOffers = this.categoriaForm.value.ofertasDisponibles;

    //     // Verifica si se han seleccionado ofertas
    //     if (selectedOffers && selectedOffers.length > 0) {
    //         // Agrega las ofertas seleccionadas a ofertasAplicadas
    //         const ofertasAplicadas = this.categoriaForm.get('ofertasAplicadas')!;
    //         for (const selectedOffer of selectedOffers) {
    //             ofertasAplicadas.value.push(selectedOffer);
    //         }

    //         console.log(ofertasAplicadas.value);

    //         // Actualiza el estado de los controles de ofertas
    //         this.categoriaForm.get('ofertasDisponibles')?.setValue(null);
    //         this.categoriaForm.get('ofertasAplicadas')?.setValue(ofertasAplicadas.value);
    //     }
    // }
