  ...
  registro() {
    ...
    if (this.categoriaForm.valid) {
      if (this.file === undefined && this.categoria.oferta === '1') {
        ...
        // this.load_btn = true;
        // Verificar si el título de la categoría ya existe antes de realizar la solicitud
    this._categoriaService.verificarExistenciaTitulo(this.categoria.titulo).subscribe(
      (tituloExiste) => {
        if (tituloExiste) {
          this.showErrorMessage('Ya existe una categoría asociada a ese título en la base de datos');
          this.load_btn = false;
        } else {
          // El título no existe, continuar con el registro de la categoría
          this._categoriaService.registro_categoria_admin(this.categoria, this.file, this.token).subscribe(
            (response) => {
