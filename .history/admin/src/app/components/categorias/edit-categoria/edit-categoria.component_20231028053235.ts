...
export class EditCategoriaComponent {
  ...
  constructor(
    ...
    this.updateForm = this.fb.group({
      nombre_categoria: [{ value: '', disabled: false }, [Validators.required, Validators.minLength(1), Validators.maxLength(25), Validators.pattern("^[a-zA-Z0-9\\sñÑ]+$")]],
      icono_categoria: [{ value: '', disabled: false }, [Validators.required, svgValidator()]],
      estado_categoria: [{ value: '', disabled: false }, [Validators.required]],
      ofertas_categoria: [{ value: '', disabled: false }],
      createdAt: { value: '', disabled: false },
      updatedAt: { value: '', disabled: false },
    });
  ...
  subscribeToFormChanges(): void {
    ...
    // this.updateForm.get("estado_categoria")!.valueChanges.subscribe((estadoValue) => {
    //   if (!estadoValue) {
    ...
    // Disable a control
    if (nombre_categoriaControl) {
      nombre_categoriaControl.disable();
    }

    // Enable a control
    if (icono_categoriaControl) {
      icono_categoriaControl.enable();
    }

    if (nombre_categoriaControl && estado_categoriaControl && icono_categoriaControl && ofertasCategoriaControl) {
      estado_categoriaControl.valueChanges.subscribe((ofertaValue) => {
        if (ofertaValue === false) {
          // this.updateForm.get('estado_categoria')?.disable(); // Enables the 'estado_categoria' control
          [nombre_categoriaControl, icono_categoriaControl, ofertasCategoriaControl].forEach(control => {
            control.disable({ onlySelf: true });

          });
        } else {
          // this.updateForm.get('estado_categoria')?.enable(); // Disables the 'estado_categoria' control
          [nombre_categoriaControl, icono_categoriaControl, ofertasCategoriaControl].forEach(control => {
