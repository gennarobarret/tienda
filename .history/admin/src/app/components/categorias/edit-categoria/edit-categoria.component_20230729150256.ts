...
export class EditCategoriaComponent {
  ...
  disableOfertaFieldsIfNeeded() {
    const ofertaValue = this.categoria.oferta;
    if (ofertaValue === 0 || ofertaValue === '0') {
      this.categoria.oferta = '0'; // Asegurarse de que el valor sea una cadena '0' en caso de ser un número 0
      this.updateForm.form.get('descuento_oferta')?.disable({ onlySelf: true });
      this.updateForm.form.get('fin_oferta')?.disable({ onlySelf: true });
      this.updateForm.form.get('fileInput')?.disable({ onlySelf: true });
    } else {
      this.updateForm.form.get('descuento_oferta')?.enable({ onlySelf: true });
      this.updateForm.form.get('fin_oferta')?.enable({ onlySelf: true });
      this.updateForm.form.get('fileInput')?.enable({ onlySelf: true });
    }
  }
  ...
  actualizar() {
    ...
    if (this.updateForm.valid) {
      ...
      if (this.categoria.oferta === '1' && (this.categoria.descuento_oferta === '0' || this.categoria.descuento_oferta === '')) {
