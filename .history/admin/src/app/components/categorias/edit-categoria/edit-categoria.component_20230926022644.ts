...
export class EditCategoriaComponent {
  ...
  constructor(
    ...
    this.categoria = {} as EditCategoria;
  }
    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      (response) => {
        ...
        if (response.data === undefined) {
          ...
          this.updateForm = this.createFormGroup(titulo, icono, estado, oferta, descuento_oferta, fin_oferta, portada);
        }
      },
      (error) => {
        console.log(error);
  ...
  createFormGroup(titulo: string, icono: string, estado: boolean, oferta: boolean, descuento_oferta: number, fin_oferta: Date, portada: File) {
    return this.fb.group({
      ...
      fin_oferta: [fin_oferta instanceof Date ? fin_oferta : new Date(), [Validators.required, this.validateOfertDate.bind(this)]],
  ...
  subscribeToFormChanges(): void {
    ...
    console.log(fechaOfertaControl);
    const portadaControl = this.updateForm.get('portada');
    console.log(portadaControl);
