...
export class EditCategoriaComponent {
  ...
  constructor(
    ...
    this.updateForm = this.fb.group({
      titulo: [null, [Validators.required, Validators.minLength(1), Validators.maxLength(25), Validators.pattern("^[a-zA-Z0-9\\sñÑ]+$")]],
      icono: [null, [Validators.required, svgValidator()]],
    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      response => {
        ...
        if (response.data === undefined) {
          ...
          // Check if the controls exist before trying to disable/enable them
          if (descuentoOfertaControl && fechaOfertaControl && portadaControl) {
            if (oferta === false) {
  ...
  subscribeToFormChanges(): void {
    ...
    const ofertaControl = this.updateForm.('oferta');
    const descuentoOfertaControl = this.updateForm.get('descuento_oferta');
    const fechaOfertaControl = this.updateForm.get('fin_oferta');
