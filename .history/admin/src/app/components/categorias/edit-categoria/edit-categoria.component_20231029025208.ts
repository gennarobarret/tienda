...
export class EditCategoriaComponent {
  ...
  subscribeToFormChanges(): void {
    ...
    // Subscribe to estado_oferta changes to toggle the control's disabled state
    this.updateForm.get('estado_oferta')?.valueChanges.subscribe((value) => {
      const ofertasControl = this.updateForm.get('ofertas_categoria');
      if (value) {
        ofertasControl?.disable();
      } else {
        ofertasControl?.enable();
      }
    });
