...
export class EditCategoriaComponent {
  ...
  actualizar() {
    ...
    console.log(this.updateForm.valid, 'updateForm.valid');
    if (this.updateForm.valid) {
      console.log(!this.showErrorEmply && !this.showErrorZero, '!this.showErrorEmply && !this.showErrorZero');
      if (!this.showErrorEmply) {
        const data: any = {};
        if (this.file) {
  ...
  resetForm() {
    ...
    this.categoria = { estado: '', oferta: '', updatedAt: this.categoria.updatedAt, createdAt: this.categoria.createdAt, descuento_oferta: '0' };
