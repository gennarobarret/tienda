...
export class EditCategoriaComponent {
  ...
  ngOnInit() {
    this._route.params.subscribe(params => {
      ...
      console.log(this.getCategoriaData);
    });
  }
  ...
  getValidDate(dateStr: any): string {
    // Check if the dateStr is a valid date in "yyyy-MM-dd" format
    const isValidDate = /^\d{4}-\d{2}-\d{2}$/.test(dateStr);
    return isValidDate ? dateStr : 'yyyy-MM-dd'; // Return the dateStr if valid, otherwise return default "yyyy-MM-dd"
  }
