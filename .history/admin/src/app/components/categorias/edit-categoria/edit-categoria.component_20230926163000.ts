    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      response => {
        ...
        if (response.data === undefined) {
          ...
          const originalDate = new Date(response.data.fin_oferta);
          // Extract year, month, and day from the Date object
          const year = originalDate.getFullYear();
          const month = String(originalDate.getMonth() + 1).padStart(2, '0'); // Months are zero-based, so add 1 and pad with '0' if necessary
          const day = String(originalDate.getDate()).padStart(2, '0');

          // Create the formatted date string in "yyyy-MM-dd" format
          const formattedDateString = `${year}-${month}-${day}`;

          console.log(formattedDateString); // Output: "0001-01-01"
          ...
          this.updateForm.patchValue({
            ...
            estado: estado,
            oferta: oferta,
            descuento_oferta: formattedDateString,
