...
export class EditCategoriaComponent {
  ...
  public selectedFileName: string = 'Seleccionar imagen';
        ...
        response => {
          if (response.data == undefined) {
            ...
            if (this.categoria && this.categoria['portada']) {
              this.selectedFileName = this.getFileNameFromPath(this.categoria['portada']);
            }
            // Check if oferta is 0 and disable the fields accordingly
  ...
  disableOfertaFields() {
    this.updateForm.form.get('descuento_oferta')?.disable();
    this.updateForm.form.get('fin_oferta')?.disable();
    this.updateForm.form.get('portada')?.disable();
  }
    ...
    if (file.size <= 4000000) {
      if (file.type == 'image/png' || file.type == 'image/webp' || file.type == 'image/jpg' || file.type == 'image/gif' || file.type == 'image/jpeg') {
        ...
        if (file) {
          ...
          this.selectedFileName = 'Seleccionar imagen';
        }
        this.file = file;
      } else {
        iziToast.show({
