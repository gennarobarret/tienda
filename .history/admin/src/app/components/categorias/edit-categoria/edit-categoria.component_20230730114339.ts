...
export class EditCategoriaComponent {
  ...
  // Remove the ngModel property and update the fileChangeEvent method
  fileChangeEvent(event: any): void {
    ...
    $('#input-img_oferta').text(file.name); // Update the label text with the file name
    this.labelSelectImagen = file.name; // Store the file name in a component property
    this.file = file; // Store the selected file in a component property
  }
  ...
  actualizar() {
    ...
    if (this.updateForm.valid && this.categoria.descuento_oferta !== '0') {
      console.log(this.showNotificationError, 'actualiza sin errores');
      const data: any = {};
      if (this.file) {
