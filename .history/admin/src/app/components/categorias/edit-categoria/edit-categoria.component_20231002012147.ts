...
export class EditCategoriaComponent {
  ...
  initializeForm() {
    ...
    this._ofertaService.listar_ofertas_admin(this.filtro, this.token).subscribe(
      response => {
        ...
        const { nombre_oferta, nombre_oferta, nombre_oferta, inicio_oferta, fin_oferta, descuento_oferta, portada_oferta, createdAt, updatedAt } = response.data;
  ...
  subscribeToFormChanges(): void {
    ...
    const estadoControl = this.updateForm.get('estado');
    // const tituloControl = this.updateForm.get('titulo');
    const iconoControl = this.updateForm.get('icono');

    if (estadoControl && iconoControl) {
      estadoControl.valueChanges.subscribe((ofertaValue) => {
        if (ofertaValue === false) {

          [iconoControl].forEach(control => {
