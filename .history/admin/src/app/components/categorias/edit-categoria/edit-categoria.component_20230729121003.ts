...
export class EditCategoriaComponent {
  ...
  public showNotificationError: boolean = false; // Add this property
  public file: File | undefined; // Add this property
  ...
  setLabelSelectImagen() {
    this.labelSelectImagen = this.categoria && this.categoria['portada'] ? this.getFileNameFromPath(this.categoria['portada']) || 'Seleccionar imagen' : 'Seleccionar imagen';
  }
  ...
  disableOfertaFields() {
    ...
    ['descuento_oferta', 'fin_oferta', 'portada'].forEach(field => {
      this.updateForm.form.get(field)?.disable({ onlySelf: true });
    });
