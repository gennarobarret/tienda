...
export class EditCategoriaComponent {
  ...
  constructor(
    ...
    this.updateForm = this.fb.group({
      ...
      ofertas_categoria: [this.ofertasAplicadas, []],
      createdAt: [''],
      updatedAt: [''],
    });
      ...
      (ofertasResponse: any) => {
        ...
        this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
          (categoriasResponse: any) => {
            ...
            console.log('ofertasDisponibles', this.ofertasDisponibles);
          },
          (error) => {
            console.log(error);
          }
    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      (response: any) => {
        ...
        if (response.data === undefined) {
          ...
          this.ofertasAplicadas = response.data.ofertas_categoria; // Asumiendo que la oferta está en una propiedad llamada "ofertas_categoria"
          console.log('ofertasAplicadas', this.ofertasAplicadas);
          this.ofertas_categoria = [...this.ofertasAplicadas, ...this.ofertasDisponibles];
          console.log('ofertas_categoria', this.ofertas_categoria);
          this.updateForm.patchValue({
            nombre_categoria: nombre_categoria,
            icono_categoria: icono_categoria,
            estado_categoria: estado_categoria,
            ofertas_categoria: this.ofertas_categoria, // Asigna la oferta aplicada al formulario
            createdAt: createdAt,
            updatedAt: updatedAt,
          });
        }
  ...
  actualizar() {
    ...
    console.info('oferta:', formValue.oferta_categoria);
