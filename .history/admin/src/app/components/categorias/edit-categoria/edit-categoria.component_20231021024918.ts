            ...
            (ofertasResponse: any) => {
              ...
              this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
                (categoriasResponse: any) => {
                  const ofertasAplicadasEnOtrasCategorias = categoriasResponse.data.map(categoria => categoria.ofertas_categoria).flat();

                  // Filtra las ofertas disponibles
                  this.ofertasDisponibles = this.ofertasDisponibles.filter(oferta => !ofertasAplicadasEnOtrasCategorias.some(aplicada => aplicada._id === oferta._id));

                  // Actualiza la variable "ofertas" utilizada por ng-multiselect-dropdown
                  this.ofertas = this.ofertasDisponibles;
                },
                (error) => {
                  console.log(error);
                }
