...
export class EditCategoriaComponent {
  ...
  public showNotificationError2: boolean = false;
    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      response => {
        ...
        if (response.data == undefined) {
          ...
          this.showNotificationError2 = this.categoria.portada === 'undefined';
  ...
  ofertaInactive() {
    this.isOfertaInactive = this.model.oferta === '0' || this.model.oferta === '';

    if (this.model.oferta === '1') {
      this.isOfertaActive = true;
    } else {
      this.isOfertaActive = false;
    }

    if (this.model.descuento_oferta === '0') {
      this.showErrorCero = false;

      if (this.isOfertaInactive) {
        this.model.descuento_oferta = '';
        this.model.fin_oferta = '';
        if (this.model.portada === 'undefined') {
  ...
  actualizar() {
    ...
    console.log(this.showNotificationError2);
    this.formSubmitted = true;
    if (this.updateForm.valid && this.categoria.descuento_oferta !== '0') {
      if (this.categoria.portada !== "undefined") {
        // this.showNotificationError2 = true;
        // console.log(this.showNotificationError2);
        // this.showFileErrorMessage('No hay una imagen para enviar');
        // return;
      }
