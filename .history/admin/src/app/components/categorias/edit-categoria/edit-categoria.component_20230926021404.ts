...
export class EditCategoriaComponent {
  ...
  initializeForm() {
    // Define the form group structure once and reuse it
    ...
    this.updateForm = this.fb.group(formGroupDefinition);
    ...
    this._categoriaService.obtener_categoria_admin(this.id, this.token).subscribe(
      response => {
        ...
        if (response.data === undefined) {
          ...
          this.updateForm.patchValue({
            ...
            oferta,
            descuento_oferta,
            fin_oferta: fin_oferta instanceof Date ? fin_oferta : new Date(),
            portada,
          });
        }
      },
      error => {
        console.log(error);
        // Handle error (e.g., navigate to an error page or display an error message)
      }
    );
  }
