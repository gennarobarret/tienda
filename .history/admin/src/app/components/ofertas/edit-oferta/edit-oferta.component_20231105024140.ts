...
export class EditOfertaComponent {
      ...
      ofertasResponse => {
        ...
        if (ofertasResponse.data === undefined) {
          ...
          const {
            ...
            createdAt,
            updatedAt
          } = ofertasResponse.data;

          const formattedInicioOferta = this.convertDateFormat(inicio_oferta);
          ...
          this.updateForm.patchValue({
            ...
            descripcion_oferta,
            descuento_oferta,
            inicio_oferta: formattedInicioOferta,
  ...
  checkSubcategories() {
    this._subcategoriaService.listar_subcategorias_admin(this.filtro, this.token).subscribe(
      subcategoriasResponse => {
        ...
        let ofertaEncontrada = this.checkOfferInSubcategories(ofertas_subcategorias);
        this.updateNivelOfertaStatus(ofertaEncontrada);
      },
      error => {
        console.log(error);
        this._router.navigate(['/panel/ofertas']);
  ...
  checkCategories() {
    this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
      (categoriasResponse: any) => {
        ...
        let ofertaEncontrada = this.checkOfferInCategories(ofertas_categorias);
        this.updateNivelOfertaStatus(ofertaEncontrada);
      },
      error => {
        console.log(error);
  ...
  subscribeToFormChanges(): void {
    ...
    if (estado_ofertaControl && nivel_ofertaControl && nombre_ofertaControl && descripcion_ofertaControl && descuento_ofertaControl && inicio_ofertaControl && fin_ofertaControl && portada_ofertaControl) {
      estado_ofertaControl.valueChanges.subscribe((ofertaValue) => {
        // if (!ofertaValue && nivel_ofertaControl.disabled) {
        //   // Si el estado de la oferta es falso y el nivel_oferta ya está deshabilitado, no hagas nada.
        //   return;
        // }

        if (ofertaValue === false && nivel_ofertaControl.disabled) {
          nivel_ofertaControl.enable({ onlySelf: true });
        } else {
          nivel_ofertaControl.disable({ onlySelf: true });
        }

        // Permitir la habilitación/deshabilitación del resto de los controles independientemente del estado del nivel_oferta.
