...
export class IndexOfertaComponent implements OnInit {
  ...
  actualizarTiempoRestante() {
    ...
    for (const oferta of this.ofertas) {
      ...
      const tiempoRestante = finOferta.getTime() - inicioOferta.getTime();
      const tiempoInicioOferta = inicioOferta.getTime() - ahora.getTime();

      if (tiempoRestante > 0) {
        if (tiempoInicioOferta === 0) {
          // Oferta está comenzando ahora
          console.log("La oferta está comenzando ahora.");
        }

        // Resto del código para calcular días, horas, minutos, segundos y actualizar oferta.tiempo_restante y oferta.texto_color
      } else {
