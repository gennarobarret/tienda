...
export class CreateOfertaComponent implements OnInit {
  ...
  initializeForm(): void {
    this.ofertaForm = this.fb.group({
      nombre_oferta: [{ value: null, disabled: true }, [Validators.required, Validators.minLength(1), Validators.maxLength(25), Validators.pattern("^[a-zA-Z0-9\\sñÑ]+$")]],
      ...
      estado_oferta: [false, [Validators.required]],
    });
  }
  ...
  subscribeToFormChanges(): void {
    ...
    const fechaFinalOfertaControl = this.ofertaForm.get('fin_oferta');
    const portadaControl = this.ofertaForm.get('portada');

    if (estado_ofertaControl && nombre_ofertaOfertaControl && descuentoOfertaControl && descripcionOfertaControl && fechaInicioOfertaControl && fechaFinalOfertaControl && portadaControl) {
      estado_ofertaControl.valueChanges.subscribe((estado_ofertaValue) => {
        if (estado_ofertaValue === false) {
          this.imageUrl = 'assets/img/default.jpg';
          this.selectedFile = null;
          [nombre_ofertaOfertaControl, descripcionOfertaControl, descuentoOfertaControl, fechaInicioOfertaControl, fechaFinalOfertaControl, portadaControl].forEach(control => {
    ...
    this._ofertaService.listar_ofertas_admin(filtro, this.token).subscribe(
      (nombre_ofertas) => {
        ...
        if (nombre_ofertaBuscado.includes(filtro)) {
          // Set the repeatedTitleError on the nombre_oferta form control
      ...
      (error) => {
        this.showErrorMessage('Ocurrió un error en al buscar nombre_oferta en base de datos');
        this.load_btn = false;
      }
    );
  }
