...
export class CreateOfertaComponent implements OnInit {
  ...
  //   }
  //   // const ofertaDateTime: Date = new Date(inputDateTimeStr);

  //   // if (isNaN(ofertaDateTime.getTime())) {
  ...
  //       // const endtDate: Date = new Date(endDateControl.value);
  ...
  //         return { 'invalidEndDate-startDate': true };
  //       }
  //     } else {
  ...
  //       return { 'invalidEndDate-CurrentDate': true };
  //     }
  ...
  private validateOfertDate(control: FormControl): { [key: string]: any } | null {
    ...
    // Validación de fecha y hora de inicio de la oferta
    if (startDateControl && startDateControl.value) {
      // error si la fecha y hora de inicio es menor que la actual
      if (startDate < currentDate) {
        this.showErrorMessage('La fecha y hora de inicio de la oferta no puede ser anterior a la fecha y hora actual.');
        return { 'invalidStartDate': true };
      }
      if (startDate > endDate) {
        this.showErrorMessage('Debes corregir la fecha finalizacion de la oferta, la fecha de finalización de la oferta no puede ser anterior a la fecha inicial.');
        return { 'invalidStartDateTime': true, 'invalidEndDateTime': true };
      }

      // si la fecha y hora de inicio es menor que la final, elimina los errores en fin_oferta
      if (endDateControl && endDateControl.value) {
        // const endDate: Date = new Date(endDateControl.value);
        if (startDate < endDate) {
          endDateControl.setErrors(null);
    ...
    // Validación de fecha y hora de finalización de la oferta
    if (endDateControl && endDateControl.value) {
      // error si la fecha y hora de final es menor que la actual
      if (endDate < currentDate) {
        this.showErrorMessage('La fecha de finalización de la oferta no puede ser anterior a la fecha actual.');
        return { 'invalidEndDateCurrentDate': true };
      }
      // error si la fecha y hora de final es menor que la actual
      if (startDate > endDate) {
        this.showErrorMessage('Debes corregir la fecha inicio de la oferta, la fecha de inicio de la oferta debe ser anterior a la fecha final.');
        return { 'invalidStartDateTime': true, 'invalidEndDateTime': true };
      }
      if (startDateControl && startDateControl.value) {
        if (endDate < startDate) {
          // error si la fecha y hora de de finalización de la oferta es mayor que la inicial
          this.showErrorMessage('La fecha de finalización de la oferta no puede ser anterior a la fecha de inicio.');
          return { 'invalidEndDate': true };
        } else {
          // si la fecha y hora de de finalización de la oferta es mayor que la inicial, elimina los errores en inicio_oferta
    ...
    // Si todas las validaciones pasan, retorna null para indicar que el input es válido
    return null;
  }
