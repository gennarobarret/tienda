...
export class CreateOfertaComponent implements OnInit {
  ...
  initializeForm(): void {
    this.ofertaForm = this.fb.group({
      nombre: [{ value: null, disabled: true, name: 'nombre' }, [Validators.required, Validators.minLength(1), Validators.maxLength(25), Validators.pattern("^[a-zA-Z0-9\\sñÑ]+$")]],
      descripcion: [{ value: null, disabled: true, name: 'descripcion' }, [Validators.required, Validators.minLength(1), Validators.maxLength(250), Validators.pattern("^[a-zA-Z0-9\\sñÑ]+$")]],
      descuento_oferta: [{ value: null, disabled: true, name: 'descuento_oferta' }, [Validators.required, this.validateDiscountRange.bind(this)]],
      inicio_oferta: [{ value: '', disabled: true, name: 'inicio_oferta' }, [Validators.required, this.validateOfertDate.bind(this)]],
      fin_oferta: [{ value: '', disabled: true, name: 'fin_oferta' }, [Validators.required, this.validateOfertDate.bind(this)]],
      portada: [{ value: null, disabled: true, name: 'portada' }, [Validators.required]],
      estado: [false, [Validators.required, name: 'estado']],
    });
  }
  ...
  subscribeToFormChanges(): void {
    ...
    const nombreOfertaControl = this.ofertaForm.get('nombre');
    const descripcionOfertaControl = this.ofertaForm.get('descripcion');
    const descuentoOfertaControl = this.ofertaForm.get('descuento_oferta');
    const fechaInicioOfertaControl = this.ofertaForm.get('inicio_oferta');
    const fechaFinOfertaControl = this.ofertaForm.get('fin_oferta');
    const portadaControl = this.ofertaForm.get('portada');

    if (estadoControl && nombreOfertaControl && descuentoOfertaControl && descripcionOfertaControl && fechaInicioOfertaControl && fechaFinOfertaControl && portadaControl) {
      estadoControl.valueChanges.subscribe((estadoValue) => {
        if (estadoValue === false) {
          this.imageUrl = 'assets/img/default.jpg';
          this.selectedFile = null;
          [nombreOfertaControl, descripcionOfertaControl, descuentoOfertaControl, fechaInicioOfertaControl, fechaFinOfertaControl, portadaControl].forEach(control => {
  ...
  get descripcion() {
    return this.ofertaForm.get("descripcion")!;
  }
  ...
  private validateOfertDate(control: FormControl): { [key: string]: any } | null {
    ...
    // const finOfertaStr: string = finOfertaControl.value;
    console.log(controlName);

    // if (!/^\d{4}-\d{2}-\d{2}$/.test(inicioOfertaStr) || !/^\d{4}-\d{2}-\d{2}$/.test(finOfertaStr)) {
    //   return { 'invalidDate': true };
    // }

    // const inicioOfertaDate: Date = new Date(inicioOfertaStr);
    // const finOfertaDate: Date = new Date(finOfertaStr);
    // const currentDate: Date = new Date(); // Current date

    // if (isNaN(inicioOfertaDate.getTime()) || isNaN(finOfertaDate.getTime())) {
    //   this.showErrorMessage('Una de las fechas introducidas no es válida.');
    //   return { 'invalidDate': true };
    // }

    // if (finOfertaDate <= inicioOfertaDate) {
    //   this.showErrorMessage('La fecha de finalización de la oferta no puede ser anterior o igual a la fecha de inicio.');
    //   return { 'invalidDate': true };
    // }

    // if (inicioOfertaDate < currentDate) {
