...
import { CategoriaService } from "src/app/services/subcategoria.service";
...
export class EditSubcategoriaComponent {
      ...
      (ofertasResponse: any) => {
        ...
        this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
          (categoriasResponse: any) => {
            const ofertasAplicadasEnOtrasCategorias: any[] = categoriasResponse.data.map((subcategoria: any) => subcategoria.ofertas_categoria).flat();
  ...
  subscribeToFormChanges(): void {
    ...
    if (nombre_categoriaControl && estado_categoriaControl && icono_categoriaControl) {
      estado_categoriaControl.valueChanges.subscribe((estadoValue) => {
