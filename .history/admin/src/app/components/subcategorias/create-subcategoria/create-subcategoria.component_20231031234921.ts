...
export class CreateSubcategoriaComponent implements OnInit {
  ...
  initializeForm(): void {
    ...
    this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
      (categoriasResponse: any) => {
        ...
        this.categorias_subcategoria.forEach(element => {
  ...
  onCategoriaSeleccionada(event: Event): void {
    ...
    const nombreCategoriaSeleccionada = (event.target as HTMLSelectElement).value;
    console.log(`ID de la oferta seleccionada: ${idCategoriaSeleccionada}`);
    console.log(`Nombre de la oferta seleccionada: ${nombreCategoriaSeleccionada}`);
    // Establecer el valor en el formulario
    // this.subcategoriaForm.patchValue({ categoria_subcategoria: idCategoriaSeleccionada });
  }
