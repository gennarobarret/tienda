  ...
  initializeForm(): void {
    ...
    this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
      (categoriasResponse: any) => {
        this.categorias_subcategoria = categoriasResponse.data;


        let ofertas_categorias: ofertas_categorias[] = [];
        this.categorias_subcategoria.forEach(categoria => {
          categoria.id = categoria._id;
          categoria.nombre_categoria = categoria.nombre_categoria;
          categoria.ofertas_categoria = categoria.ofertas_categoria;
          this.ofertas_categorias = this.ofertas_categorias.concat(categoria.ofertas_categorias);
        });
        // Explicitly define todasLasOfertas as an array of 'Offer' type

        // Iterate through the categories and concatenate their offers
        this.categorias_subcategoria.forEach(categoria => {
          ofertas_categorias = ofertas_categorias.concat(categoria.ofertas_categoria);
        });

        console.log(ofertas_categorias);



        // this.ofertas_categorias.forEach(offer => {
        //   console.log("Offer ID:", offer._id);
        //   console.log("Offer Name:", offer.nombre_oferta);
        //   console.log("Discount:", offer.descuento_oferta);
        //   console.log("Description:", offer.descripcion_oferta);
