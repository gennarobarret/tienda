...
export class CreateSubcategoriaComponent implements OnInit {
  ...
  onCategoriaSeleccionada(event: Event): void {
    ...
    if (selectedCategory) {
      ...
      this.ofertasDisponibles = [...this.ofertasDisponibles, ...selectedCategory.ofertas_categoria];
      // console.log('ofertas_subcategoria', this.ofertas_categoria);
  ...
  public registro(): void {
    ...
    if (!Array.isArray(formValue.ofertas_subcategoria)) {
      formValue.ofertas_subcategoria = []; // Convierte a un array vacío si no es un array
    }
      ...
      (nombre_subcategorias) => {
        ...
        if (nombre_subcategoriaBuscado.includes(filtro)) {
          ...
          this._subcategoriaService.registro_subcategoria_admin(formValue, this.token).subscribe(
            ...
            (error) => {
              if (error.status === 409 && error.error.message === 'Ya existe una subcategoría con el mismo título.') {
                this.showErrorMessage('Ya existe una subcategoría asociada a ese título en la base de datos');
                this.subcategoriaForm.controls['nombre_subcategoria'].setErrors({ 'repeatedTitleError': true });
              } else {
