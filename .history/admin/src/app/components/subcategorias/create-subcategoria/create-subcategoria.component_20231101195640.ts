...
export class CreateSubcategoriaComponent implements OnInit {
  ...
  initializeForm(): void {
    ...
    this._categoriaService.listar_categorias_admin(this.filtro, this.token).subscribe(
      (categoriasResponse: any) => {
        ...
        console.log(this.categorias_subcategoria);
  ...
  onCategoriaSeleccionada(event: Event): void {
    const selectedCategoryId = this.subcategoriaForm.get('selectedCategory')?.value;
    const selectedCategory = this.subcategoriaForm.find(category => category._id === selectedCategoryId);

    // You can update the offersFormControl with the selected category's offers
    if (selectedCategory) {
      const offers = selectedCategory.ofertas_categoria.map(offer => ({
        id: offer._id,
        name: offer.nombre_oferta
      }));
      // Set the options for the offers FormControl
      this.subcategoriaForm.setControl('selectedOffer', this.formBuilder.control(''));
      this.subcategoriaForm.setControl('selectedOffer', this.formBuilder.control('', /* Other Validators */));
      this.subcategoriaForm.get('selectedOffer')?.setValue(''); // Clear previous selection
    }
