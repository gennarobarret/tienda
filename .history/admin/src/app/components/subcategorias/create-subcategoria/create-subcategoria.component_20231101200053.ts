...
export class CreateSubcategoriaComponent implements OnInit {
  ...
  onCategoriaSeleccionada(event: Event): void {
    const selectedCategoryId = this.subcategoriaForm.get('categoria_subcategoria')?.value;
    console.log(selectedCategoryId);
    const selectedCategory = this.categorias_subcategoria.find(category => category._id === selectedCategoryId);
    console.log(selectedCategory);
    if (selectedCategory) {
      this.ofertas_subcategoria = selectedCategory.ofertas_categoria;
      console.log(this.ofertas_subcategoria);
    } else {
      this.ofertas_subcategoria = []; // Reset the offers when no category is selected
    }
  }
